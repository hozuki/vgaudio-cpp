    ...
    do { \
            ...
            switch (mode) {
                case 0: {
                    ...
                    for (auto i = 0; i < length; i += 1) {
                        ...
                        tie(typeIndex, wrapper) = ArrayUnpacker_UnpackArray(reader, outType, rank - 1);
                        array[i] = wrapper;
                    }
                ...
                case 2: {
                    ...
                    for (auto i = 0; i < length; i += 1) {
                        TypeIndex typeIndex;
                        ArrayWrapper<> wrapper;
                        tie(typeIndex, wrapper) = ArrayUnpacker_ReadArray(reader, storedType, outType, lengths[i]);
                        array[i] = wrapper;
                    }
        ...
        TArrayPack ArrayUnpacker_UnpackInternal(TypeIndex outType, TypeIndex storedType, const shared_ptr<BinaryReader> &reader, int32_t depth, const array_ptr<int32_t> &dimensions) {
            ...
            for (auto i = 0; i < length; i += 1) {
                TypeIndex typeIndex;
                ArrayWrapper<> wrapper;
                tie(typeIndex, wrapper) = ArrayUnpacker_UnpackInternal(outType, storedType, reader, depth + 1, dimensions);
                array[i] = wrapper;
            }
